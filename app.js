...
async function setupEnergyDashboard(user, preferences, contextOverride) {
  ...
  const pendingModalEl = contentEl.querySelector("[data-pending-modal]");
  const pendingBackdropEl = contentEl.querySelector("[data-pending-backdrop]");
  const pendingCloseButton = contentEl.querySelector("[data-pending-close]");
  const pendingSubtitleEl = contentEl.querySelector("[data-pending-subtitle]");
  const pendingListEl = contentEl.querySelector("[data-pending-list]");
  const pendingEmptyEl = contentEl.querySelector("[data-pending-empty]");
  const pendingMessageEl = contentEl.querySelector("[data-pending-message]");
  const pendingAcceptAllButton = contentEl.querySelector(
    "[data-pending-accept-all]"
  );
  const pendingDeclineAllButton = contentEl.querySelector(
  ...
  const lateReplyFineConfig = organizationSettings.fines?.lateReply ?? {};
  pendingMovesState.fineEnabled = Boolean(lateReplyFineConfig.enabled);
  pendingMovesState.fineDays = Number.parseInt(lateReplyFineConfig.days ?? 0, 10) || 0;
  pendingMovesState.fineAmount =
    normalizeCostValue(lateReplyFineConfig.amount) ?? 0;
  const addPhotoState = {
    tools: [],
    filtered: [],
    filters: {
  ...
  if (pendingBackdropEl) {
    pendingBackdropEl.addEventListener("click", closePendingModal);
  }
  if (pendingCloseButton) {
    pendingCloseButton.addEventListener("click", closePendingModal);
  }
  pendingModalEl?.addEventListener("keydown", (event) => {
    if (event.key === "Escape") {
      closePendingModal();
    }
  });
  ...
  const setPendingSubtitle = (text) => {
    if (pendingSubtitleEl) {
      pendingSubtitleEl.textContent = text;
    }
  };

  const setPendingMessage = (text = "", type = "") => {
    if (!pendingMessageEl) return;
    pendingMessageEl.textContent = text;
    pendingMessageEl.classList.remove("is-error", "is-success", "is-info");
    if (type) {
      pendingMessageEl.classList.add(`is-${type}`);
    }
  };
  ...
  const calculateLateReplyFine = (moveDateValue) => {
    if (!pendingMovesState.fineEnabled) return null;
    const amount = pendingMovesState.fineAmount;
    const moveDate = parseDateValue(moveDateValue);
    if (!moveDate || !Number.isFinite(amount)) return 0;
    const daysPassed = diffDays(moveDate, new Date());
    if (daysPassed <= pendingMovesState.fineDays) return 0;
    const overdueDays = daysPassed - pendingMovesState.fineDays;
    const fineDays = Math.max(0, overdueDays - 1);
    return fineDays * amount;
  };

  const resolvePendingTool = (tools, move) => {
    const number = normalizeToolNumberValue(move?.["Номер"] ?? "");
    const accounting = String(move?.["Бух.номер"] ?? "").trim();
    if (number) {
      const byNumber = tools.find(
        (tool) => normalizeToolNumberValue(tool?.["Номер"] ?? "") === number
      );
      if (byNumber) return byNumber;
    }
    if (accounting) {
      return tools.find(
        (tool) => String(tool?.["Бух.номер"] ?? "").trim() === accounting
      );
    }
    return null;
  };

  const renderPendingMovesTable = (items) => {
    const table = document.createElement("div");
    table.className = "tools-table tools-table--pending";
    if (pendingMovesState.fineEnabled) {
      table.classList.add("tools-table--pending-fines");
    }
    ...
    items.forEach((item) => {
      ...
      const row = document.createElement("div");
      row.className = "tools-table__row";
      row.dataset.pendingIndex = String(item.__moveIndex);

      const numberCell = document.createElement("div");
      numberCell.className = "tools-table__cell tools-table__cell--number";
      const number = String(tool?.["Номер"] ?? move?.["Номер"] ?? "").trim();
      const accountingNumber = String(
        tool?.["Бух.номер"] ?? move?.["Бух.номер"] ?? ""
      ).trim();
      numberCell.textContent = number || accountingNumber || "—";
      ...
      const model = String(tool?.["Модель"] ?? "").trim();
      const metaLines = [];
      const primaryMeta = [manufacturer, model].filter(Boolean).join(" · ");
      if (primaryMeta) {
        metaLines.push(primaryMeta);
      }
      const movedBy = String(move?.["Переместил"] ?? "").trim();
      const targetObject = String(move?.["Новый объект"] ?? "").trim();
      const movedAt = String(move?.["Дата перемещения"] ?? "").trim();
      metaLines.push(`Отправил: ${movedBy || "—"}`);
      if (targetObject) {
        metaLines.push(`Новый объект: ${targetObject}`);
      }
      if (accountingNumber) {
        metaLines.push(`Бух.номер: ${accountingNumber}`);
      }
      if (movedAt) {
        metaLines.push(`Дата: ${movedAt}`);
      }
      ...
      let fineCell = null;
      if (pendingMovesState.fineEnabled) {
        fineCell = document.createElement("div");
        fineCell.className = "tools-table__cell";
        const fineWrap = document.createElement("div");
        fineWrap.className = "pending-fine";
        const fineLabel = document.createElement("div");
        fineLabel.textContent = "Штраф";
        const fineValue = document.createElement("div");
        fineValue.className = "pending-fine__value";
        const fine = item.fine ?? 0;
        fineValue.textContent = formatNotificationCost(fine);
        fineWrap.append(fineLabel, fineValue);
        fineCell.appendChild(fineWrap);
      }
      ...
      acceptButton.className = "pending-action pending-action--accept";
      acceptButton.dataset.pendingAction = "accept";
      acceptButton.dataset.pendingIndex = String(item.__moveIndex);
      acceptButton.textContent = "✅";
      acceptButton.setAttribute("aria-label", "Принять");
      acceptButton.title = "Принять";
      const declineButton = document.createElement("button");
      declineButton.type = "button";
      declineButton.className = "pending-action pending-action--decline";
      declineButton.dataset.pendingAction = "decline";
      declineButton.dataset.pendingIndex = String(item.__moveIndex);
      declineButton.textContent = "✕";
      declineButton.setAttribute("aria-label", "Не принять");
      declineButton.title = "Не принять";
      ...
      img.className = "tools-table__thumb-image";
      img.alt = name || "Инструмент";
      const photoNumber = resolveToolPhotoNumber(tool ?? move);
      const photoCandidates = photoNumber
        ? buildToolPhotoCandidates(context.orgFolderName, photoNumber)
        : [];
      let candidateIndex = 0;
      const tryCandidate = () => {
        ...
        const next = photoCandidates[candidateIndex];
        candidateIndex += 1;
        img.src = next;
      };
      img.onerror = () => {
      ...
      row.append(actionsCell, photoCell);
      table.appendChild(row);
    });
  ...
  const renderPendingMovesList = () => {
    ...
    clearPendingList();
    const items = pendingMovesState.items;
    if (items.length) {
      pendingListEl.appendChild(renderPendingMovesTable(items));
    }
    if (pendingEmptyEl) {
      pendingEmptyEl.classList.toggle("is-hidden", items.length > 0);
    }
    if (pendingAcceptAllButton) {
      pendingAcceptAllButton.disabled =
        pendingMovesState.isSaving || items.length === 0;
    }
    if (pendingDeclineAllButton) {
      pendingDeclineAllButton.disabled =
        pendingMovesState.isSaving || items.length === 0;
    }
    setPendingSubtitle(`На принятии: ${items.length}`);
    updateEnergyPendingStat(items.length);
  };

  const loadPendingMovesList = async () => {
    const orgFolder = context.orgFolderName ?? "";
    pendingMovesState.orgFolder = orgFolder;
    if (!orgFolder) {
      pendingMovesState.items = [];
      setPendingSubtitle("Не удалось определить организацию.");
    ...
    let tools = [];
    try {
      tools = await loadToolsData(orgFolder);
    } catch (error) {
      tools = [];
    }

    const userKey = normalizePersonName(user?.full_name ?? "");
    pendingMovesState.items = movesData
      .map((move, index) => {
        const responseDate = String(move?.["Дата ответа"] ?? "").trim();
        if (responseDate) return null;
        const acceptedBy = normalizePersonName(move?.["Принял"] ?? "");
        if (!acceptedBy || acceptedBy !== userKey) return null;
        const tool = resolvePendingTool(tools, move);
        const fine = calculateLateReplyFine(move?.["Дата перемещения"]);
        return {
          move,
          tool,
          fine,
          __moveIndex: index,
          __senderKey: normalizePersonName(move?.["Переместил"] ?? ""),
          __numberKey: normalizeToolNumberValue(move?.["Номер"] ?? ""),
        };
      })
      .filter(Boolean)
      .sort((a, b) => {
        const senderCompare = a.__senderKey.localeCompare(b.__senderKey, "ru");
        if (senderCompare !== 0) return senderCompare;
        return a.__numberKey.localeCompare(b.__numberKey, "ru", {
          numeric: true,
        });
      });
  ...
  const openPendingModal = async () => {
    ...
    pendingModalEl.classList.remove("is-hidden");
    document.body.style.overflow = "hidden";
    setPendingMessage("");
    setPendingSubtitle("Загружаем список...");
    await loadPendingMovesList();
  };

  const closePendingModal = () => {
    if (!pendingModalEl) return;
    pendingModalEl.classList.add("is-hidden");
    document.body.style.overflow = "";
    setPendingMessage("");
  };

  const findMoveIndex = (moves, item) => {
    const number = normalizeToolNumberValue(item.move?.["Номер"] ?? "");
    const accounting = String(item.move?.["Бух.номер"] ?? "").trim();
    const moveDate = String(item.move?.["Дата перемещения"] ?? "").trim();
    const movedBy = normalizePersonName(item.move?.["Переместил"] ?? "");
    const acceptedBy = normalizePersonName(item.move?.["Принял"] ?? "");
    return moves.findIndex((move) => {
      const responseDate = String(move?.["Дата ответа"] ?? "").trim();
      if (responseDate) return false;
      if (
        normalizePersonName(move?.["Принял"] ?? "") !== acceptedBy ||
        normalizePersonName(move?.["Переместил"] ?? "") !== movedBy
      ) {
        return false;
      }
      ...
      if (moveDate && moveDateValue && moveDate !== moveDateValue) return false;
      if (number && moveNumber) {
        return moveNumber === number;
      }
      if (accounting && moveAccounting) {
        return moveAccounting === accounting;
      }
      return false;
    });
  };

  const respondToPendingMoves = async (items, accepted) => {
    if (pendingMovesState.isSaving) return;
    if (!items.length) return;
    pendingMovesState.isSaving = true;
    setPendingMessage("Сохраняем ответ...", "info");
    renderPendingMovesList();
    const orgFolder = context.orgFolderName ?? "";
    const movesPath = `./${orgFolder}/Перемещения.json`;
    const toolsPath = `./${orgFolder}/База с инструментами.json`;
    ...
    let toolsData = [];
    if (accepted) {
      toolsData = await loadToolsData(orgFolder);
    }

    const now = new Date();
    const responseDate = formatDateValue(now);
    const responseLabel = accepted ? "Принял" : "Не принял";
    let updatedMovesCount = 0;
    let updatedToolsCount = 0;

    items.forEach((item) => {
      const moveIndex = findMoveIndex(movesData, item);
      ...
      if (accepted && toolsData.length) {
        const numberKey = normalizeToolNumberValue(move?.["Номер"] ?? "");
        const accountingKey = String(move?.["Бух.номер"] ?? "").trim();
        const toolIndex = toolsData.findIndex((tool) => {
          const toolNumber = normalizeToolNumberValue(tool?.["Номер"] ?? "");
          const toolAccounting = String(tool?.["Бух.номер"] ?? "").trim();
          if (numberKey && toolNumber) return numberKey === toolNumber;
          if (accountingKey && toolAccounting) {
            return accountingKey === toolAccounting;
          }
          return false;
        });
        if (toolIndex >= 0) {
          const updatedTool = { ...toolsData[toolIndex] };
          const newObject = String(move?.["Новый объект"] ?? "").trim();
          if (newObject) {
            updatedTool["Объект"] = newObject;
          }
          const acceptedName = String(user?.full_name ?? "").trim();
          if (acceptedName) {
            updatedTool["Ответственный"] = acceptedName;
          }
          toolsData[toolIndex] = updatedTool;
          updatedToolsCount += 1;
        }
      }
    });
    ...
    try {
      await saveJson(movesPath, movesData, { user });
      if (accepted && updatedToolsCount > 0) {
        await saveJson(toolsPath, toolsData, { user });
      }
      setPendingMessage(
        updatedMovesCount
          ? `Готово: ${responseLabel.toLowerCase()} ${updatedMovesCount} шт.`
          : "Не удалось обновить запросы.",
        updatedMovesCount ? "success" : "error"
      );
    } catch (error) {
      console.error(error);
      setPendingMessage("Не удалось сохранить ответы.", "error");
    } finally {
  ...
  if (pendingAcceptAllButton) {
    pendingAcceptAllButton.addEventListener("click", () => {
      respondToPendingMoves(pendingMovesState.items, true);
    });
  }

  if (pendingDeclineAllButton) {
    pendingDeclineAllButton.addEventListener("click", () => {
      respondToPendingMoves(pendingMovesState.items, false);
    });
  }
  ...
  if (pendingListEl) {
    pendingListEl.addEventListener("click", (event) => {
      ...
      const index = Number.parseInt(button.dataset.pendingIndex ?? "", 10);
      if (!Number.isFinite(index)) return;
      const item = pendingMovesState.items.find(
        (entry) => entry.__moveIndex === index
      );
      if (!item) return;
      respondToPendingMoves([item], action === "accept");
    });
  }

  if (energyPendingStatEl && pendingModalEl && !energyPendingStatEl.dataset.pendingListener) {
    energyPendingStatEl.addEventListener("click", () => {
      if (!pendingModalEl.classList.contains("is-hidden")) {
        closePendingModal();
        return;
      }
      openPendingModal();
    });
    energyPendingStatEl.dataset.pendingListener = "true";
  }
